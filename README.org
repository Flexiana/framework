...
*** Interceptors
    ...
    *state* map and must return a wrapped container *state* map.
    ...
    default-interceptors, but it's also possible to set peer route.
    ...
    We provide tree strategies to combine the peer route interceptors with
    the defaults one: around, inside, override.

    - *Override* means just execute the interceptors defined in the route.

      [[./resources/images/override.png]]

    - *Around* means execute this the route interceptors around the default
      ones, i.e:
