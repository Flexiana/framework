...
*** QuickStart
    ...
    change the current working directory. The second command calls ~auto.sh~
    ...
    host and docker daemon should be initialize a priory, otherwise this whole
    chain of commands will fail.

    It should also be noted that after the first installation everything will
    be cached preventing unnecessary rework to happen, it's possible to run only
...
*** Monads
    ...
    its data value (structure), and every function that returns
...
*** Config
    ...
    robust, and extensible mechanisms to manager our configuration
    data.

    So we use the same well know ~.edn~ file map to simplify
