...
(defn one-user
  ...
  (c/response state {:view-type "single user"
...
(defn multi-user
  [{response-data :response-data :as state}]
  (c/response state {:view-type "multiple users"
                     :data      {:users (->> response-data
                                             :db-data
                                             (map ->user-view))}}))
...
(defn fetch-posts-comments
  [{{{id :id} :query-params} :request
    ...
    :as                      state}]
  (if id
    (c/response state {:view-type "single user with posts and comments"
                       :data      {:users (render-posts-with-comments (:db-data response-data))}})
    (c/response state {:view-type "multiple users with posts and comments"
                       :data      {:users (render-posts-with-comments (:db-data response-data))}})))
...
(defn fetch-posts
  [{{{id :id} :query-params} :request
    ...
    :as                      state}]
  (if id
    (c/response state {:view-type "single user with posts"
                       :data      {:users (render-posts (:db-data response-data))}})
    (c/response state {:view-type "multiple users with posts"
                       :data      {:users (render-posts (:db-data response-data))}})))
