...
(deftest administrator-on-items
  (is (true? (acl (add-user-by-id {} 2) "SELECT * FROM items;")))
  (is (true? (acl (add-user-by-id {} 2) (-> (select [:*])
                                            (from :items)))))
  (is (true? (acl (add-user-by-id {} 2) "INSERT INTO items ;")))
  (is (true? (acl (add-user-by-id {} 2) (insert-into "items"))))
  (is (true? (acl (add-user-by-id {} 2) "DELETE FROM items WHERE id EQ 125;")))
  (is (true? (acl (add-user-by-id {} 2) (-> (delete-from "items")
                                            (where [:= :id 125])))))
  (is (true? (acl (add-user-by-id {} 2) "UPDATE items WHERE id EQ 123;")))
  (is (true? (acl (add-user-by-id {} 2) (-> (update "items")
                                            (where [:= :id 123]))))))

(deftest administrator-on-users
  (is (true? (acl (add-user-by-id {} 2) "SELECT * FROM users WHERE id EQ 1")))
  (is (true? (acl (add-user-by-id {} 2) (-> (select :*)
                                            (from [:users :u])
                                            (where [:= :u.id 1])))))
  (is (true? (acl (add-user-by-id {} 2) "SELECT * FROM users;")))
  (is (true? (acl (add-user-by-id {} 2) (-> (select :*)
                                            (from [:users :u])))))
  (is (true? (acl (add-user-by-id {} 2) "SELECT * FROM users WHERE user-id EQ 2")))
  (is (true? (acl (add-user-by-id {} 2) (-> (select :*)
                                            (from [:users :u])
                                            (where [:= :u.id 2])))))
  (is (true? (acl (add-user-by-id {} 2) "INSERT INTO users")))
  (is (true? (acl (add-user-by-id {} 2) (insert-into "users"))))
  (is (true? (acl (add-user-by-id {} 2) "DELETE FROM users WHERE id EQ 1")))
  (is (true? (acl (add-user-by-id {} 2) (-> (delete-from :users)
                                            (where [:= :id 1])))))
  (is (true? (acl (add-user-by-id {} 2) "DELETE FROM users WHERE users.id EQ 2")))
  (is (true? (acl (add-user-by-id {} 2) (-> (delete-from :users)
                                            (where [:= :users.id 2])))))
  (is (true? (acl (add-user-by-id {} 2) "DELETE FROM users WHERE id EQ 2")))
  (is (true? (acl (add-user-by-id {} 2) (-> (delete-from :users)
                                            (where [:= :id 2])))))
  (is (true? (acl (add-user-by-id {} 2) "UPDATE users WHERE user-id EQ 1")))
  (is (true? (acl (add-user-by-id {} 2) (-> (update :users)
                                            (where [:= :id 1])))))
  (is (true? (acl (add-user-by-id {} 2) "UPDATE users WHERE user-id EQ 2")))
  (is (true? (acl (add-user-by-id {} 2) (-> (update :users)
                                            (where [:= :id 2]))))))

(deftest administrator-on-addresses
  (is (true? (acl (add-user-by-id {} 2) "SELECT * FROM addresses WHERE id EQ 1")))
  (is (true? (acl (add-user-by-id {} 2)
                  (-> (select :*)
                      (from :addresses)
                      (where [:= :id 1])))))
  (is (true? (acl (add-user-by-id {} 2) "UPDATE addresses WHERE user-id EQ 1;")))
  (is (true? (acl (add-user-by-id {} 2)
                  (-> (update :*)
                      (from :addresses)
                      (where [:= :user-id 1])))))
  (is (= true
         (acl (add-user-by-id {} 2)
              "SELECT * FROM addresses;")
         (acl (add-user-by-id {} 2)
              (-> (select :*)
                  (from :addresses)))
