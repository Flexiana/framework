...
;        username (:users/username (first (db.sql/execute (get-in state [:db :datasource]) sql-map)))
;        expected "admin"]
...
;  (let [sql-map ""
;        result (db.sql/execute (get-in state [:db :datasource]) sql-map)
;        expected {}]
...
;        role (-> (db.sql/execute! (get-in state [:db :datasource]) sql-vec)
;                 (first)
;                 (:users/role))
;        expected "user"]
...
;  (let [sql-vec nil
;        result (db.sql/execute! (get-in state [:db :datasource]) sql-vec)
;        expected {}]
;    ;; verify if the retrieved result is the expected
;    (is (= result expected))))
;
